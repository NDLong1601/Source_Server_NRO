...
description: Use for an explicit security review or hardening task, or when a requested change crosses an untrusted-input, authentication, JDBC/data-storage, TCP protocol, HTA/JScript, PowerShell, filesystem, external-service, dependency, secret, or privacy boundary. In this repository, NRO-specific skills remain workflow owners and HTTP-only controls apply only to real HTTP surfaces.
---
...
- Read the root `AGENTS.md` first. Repository-specific rules and NRO-specific skills override this general workflow.
- Treat the TCP game protocol, command dispatch, JDBC/MySQL queries, HTA/JScript input, filesystem paths, PowerShell arguments, and generated client data as the primary trust boundaries here.
- HTTP-only controls in this skill (cookies, CORS, CSP, HTTP headers, Express examples) apply only when reviewing a real HTTP surface. Do not introduce a web stack to satisfy an inapplicable checklist item.
- This repository uses Java 17 and Ant/NetBeans with JAR dependencies under `lib/`; do not assume npm, Maven, Gradle, or a committed package-manager lockfile.
- A security review is read-only unless the user asks for implementation. Do not expose secret values while scanning, and do not build/deploy `20.jar`, restart services, or mutate databases without that authority.
- Preserve the NRO packet limit of 65,535 bytes and the cache/relogin verification rules in root `AGENTS.md`.
...
- **Audit dependencies at their actual installation boundary** before release. For this repository, inventory JARs under `lib/`, verify source/version/license/provenance, and check relevant advisories; use a package-manager audit only inside a nested project that actually owns that manager and lockfile.
...
These are prevention patterns, not a ranking. Apply only the controls relevant to the repository's actual trust boundaries and use this skill's review checklist as the quick reference.
...
Before a destructive call, require all three: the resolved target sits under an **allowlisted root** (compare after resolving symlinks, never on the raw string); it is at least one level **below** that root, so a root is never itself the target; and it carries **evidence that it is yours**, read *before* the operation and before any teardown that removes it — otherwise "absent" and "not mine" are indistinguishable. On refusal, log the rejected target and stop: a cleanup that falls back to a broader default path is the failure this guards against. Root `AGENTS.md` supplies the repository-specific destructive-action policy.
...
1. **Find the installation boundary and manager.** At this repository root, inventory the JARs under `lib/` and corroborate their versions and provenance with the build files and release process. For an independent nested project, use its owning manifest, lockfile, package-manager declaration, and CI; stop on disagreement or competing lockfiles.
...
**Always check before committing:** use a secret scanner configured to redact matched values, or inspect staged filenames and redacted match metadata. Never print suspected secret contents into command output, logs, chat, or review comments.
...
For repository-specific security, pre-commit, destructive-action, build, and deployment rules, see root `AGENTS.md` and `BUILD_JAVA_STANDARD.md`.
