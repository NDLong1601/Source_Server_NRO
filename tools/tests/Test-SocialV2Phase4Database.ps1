...
$tempRoot = Join-Path $repoRoot ('target\social-v2-phase4-db-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $testSource = Join-Path $PSScriptRoot 'SocialProfileDatabaseIntegrationTest.java'
    ...
    & java "-Dsocial.v2.test.jdbcUrl=$JdbcUrl" "-Dsocial.v2.test.dbUser=$DbUser" `
        -cp ($classes + ';' + $classpath) nro.models.social.SocialProfileDatabaseIntegrationTest
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host 'SOCIAL_V2_PHASE4_DATABASE_OK'
} finally {
    if (Test-Path -LiteralPath $tempRoot) {
        ...
        if (-not $resolvedTemp.StartsWith($allowedRoot, [StringComparison]::OrdinalIgnoreCase)) {
            throw 'Refusing Social V2 phase-4 database test cleanup outside repository target'
        }
