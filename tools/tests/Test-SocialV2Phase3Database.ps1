...
$tempRoot = Join-Path $repoRoot ('target\social-v2-phase3-db-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $testSource = Join-Path $PSScriptRoot 'SocialDirectoryDatabaseIntegrationTest.java'
    ...
    & java "-Dsocial.v2.test.jdbcUrl=$JdbcUrl" "-Dsocial.v2.test.dbUser=$DbUser" `
        -cp ($classes + ';' + $classpath) nro.models.social.SocialDirectoryDatabaseIntegrationTest
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host 'SOCIAL_V2_PHASE3_DATABASE_OK'
} finally {
    if (Test-Path -LiteralPath $tempRoot) {
        ...
        if (-not $resolvedTemp.StartsWith($allowedRoot, [StringComparison]::OrdinalIgnoreCase)) {
            throw 'Refusing Social V2 phase-3 database test cleanup outside repository target'
        }
