...
$tempRoot = Join-Path $repoRoot ('target\social-v2-phase3-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($redExit -eq 0 -or ($redOutput -join "`n") -notmatch 'Social V2 RED specifications still failing: 2') {
        throw 'Phase-3 rules must turn search green; chat and location must remain RED.'
    }
    Write-Output 'SOCIAL_V2_PHASE3_OK'
} finally {
    if (Test-Path -LiteralPath $tempRoot) {
        ...
        if (-not $resolvedTemp.StartsWith($allowedRoot, [StringComparison]::OrdinalIgnoreCase)) {
            throw 'Refusing Social V2 phase-3 test cleanup outside repository target'
        }
