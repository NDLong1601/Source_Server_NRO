...
from workspace_layout import map_workspace_root
...
def get_projects_root(repo_root: Path, projects_root: Path | None = None) -> Path:
    return (projects_root or map_workspace_root(repo_root) / "projects").resolve()


def get_releases_root(repo_root: Path, releases_root: Path | None = None) -> Path:
    return (releases_root or map_workspace_root(repo_root) / "releases").resolve()
...
def get_build_paths(repo_root: Path, project_id: str) -> tuple[Path, Path, Path]:
    output = map_workspace_root(repo_root) / "output" / "projects" / validate_project_id(project_id)
