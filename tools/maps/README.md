...
`tools/maps` là lõi project/biên dịch map độc lập dành cho admin map editor. Compiler luôn tạo bundle staging trước; Giai đoạn 2 bổ sung project draft, import map runtime, preview trong tab **Tạo Map** và publish có kiểm tra/rollback.
...
```powershell
python .\tools\maps\scripts\compile_map_plan.py .\tools\maps\examples\stage1-demo\teamobi-map-plan.json
```

Kết quả mặc định nằm tại `tools/maps/output/map-<id>`:
...
```powershell
python .\tools\maps\scripts\compile_map_plan.py .\tools\maps\examples\stage1-demo\teamobi-map-plan.json --force
```
...
- publish tile/background + `map_template` + tăng `data/map/version.txt`, đồng thời lưu snapshot rollback trong `tools/maps/releases`.

Draft nằm trong `tools/maps/projects` và build nằm trong `tools/maps/output/projects`. Hai vùng này tách biệt hoàn toàn khỏi `data/map` cho tới khi người quản trị bấm **Publish** và xác nhận.

CLI tương ứng:

```powershell
python .\tools\maps\scripts\manage_map_projects.py --repo-root . list
python .\tools\maps\scripts\manage_map_projects.py --repo-root . create --map-id 186 --name "Map mới" --width 40 --height 20 --tile-set-id 2
python .\tools\maps\scripts\manage_map_projects.py --repo-root . compile --project-id map-186
python .\tools\maps\scripts\manage_map_projects.py --repo-root . release-plan --project-id map-186
```
...
```powershell
python -m unittest discover -s .\tools\maps\tests -p "test_*.py" -v
```
...
Mỗi lần publish thành công tạo một release v2 bất biến trong `tools/maps/releases/<projectId>/<releaseId>` gồm:
...
```powershell
python .\tools\maps\scripts\manage_map_projects.py --repo-root . list-releases --project-id map-186
python .\tools\maps\scripts\manage_map_projects.py --repo-root . get-release --project-id map-186 --release-id <release-id>
python .\tools\maps\scripts\manage_map_projects.py --repo-root . verify-release --project-id map-186 --release-id <release-id>
```
...
```powershell
python .\tools\maps\scripts\manage_map_projects.py --repo-root . get-acceptance --project-id map-186 --release-id <release-id>
python .\tools\maps\scripts\manage_map_projects.py --repo-root . save-acceptance --project-id map-186 --release-id <release-id> --acceptance-json <qa.json>
```
