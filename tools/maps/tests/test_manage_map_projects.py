...
MAP_WORKSPACE_ROOT = TESTS_ROOT.parent
REPO_ROOT = MAP_WORKSPACE_ROOT.parent.parent
MANAGER_PATH = MAP_WORKSPACE_ROOT / "scripts" / "manage_map_projects.py"
...
class ManageMapProjectsTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        workspace = manager.map_workspace_root(self.repo)
        (workspace / "schemas").mkdir(parents=True)
        shutil.copy2(
            MAP_WORKSPACE_ROOT / "schemas" / "teamobi-map-plan.schema.json",
            workspace / "schemas" / "teamobi-map-plan.schema.json",
        )
    ...
    def test_publish_runtime_creates_rollback_and_restores_exact_state(self) -> None:
        ...
        self.assertTrue((manager.map_workspace_root(self.repo) / "releases" / ".publish.lock").is_file())
        ...
        self.assertFalse(tile_target.exists())
        self.assertFalse(bg_target.exists())
        self.assertFalse((manager.map_workspace_root(self.repo) / "releases" / ".publish.lock").exists())
    ...
    def test_custom_x4_asset_compiles_publishes_and_rolls_back_all_zooms(self) -> None:
        ...
        for zoom, size in expected_sizes.items():
            path = manager.map_workspace_root(self.repo) / "projects" / project_id / "assets" / zoom / "500.png"
        ...
        manager.save_project(self.repo, project_id, plan)
        manager.compile_project(self.repo, project_id)
        build = manager.map_workspace_root(self.repo) / "output" / "projects" / project_id
    ...
    def test_fine_offset_runtime_template_reuses_existing_png_without_asset_copy(self) -> None:
        ...
        generated_registry = manager.load_generated_offset_registry(
            manager.map_workspace_root(self.repo) / "projects" / project_id
        )
        ...
        second_build = (
            manager.map_workspace_root(self.repo) / "output" / "projects" / second["project"]["projectId"]
        )
