...
$SqlPath = Join-Path $Root "sql\activity_config.sql"
...
function Find-Executable {
    ...
    if (-not $command) { throw "Cannot find $Name in PATH." }
...
if ($serverProcesses.Count -gt 0) {
    throw "Stop the server before applying the Activity Phase 1 schema. Active PIDs: $($serverProcesses.ProcessId -join ', ')"
}
...
$backupPath = Join-Path (Join-Path $Root "backups") ("activity_phase1_" + $stamp)
if (Test-Path -LiteralPath $backupPath) { throw "Backup path already exists: $backupPath" }
New-Item -ItemType Directory -Path $backupPath | Out-Null

$dumpPath = Join-Path $backupPath ("$($database.Name)_before_activity_phase1.sql")
$dumpArgs = @(
...
$jarBackupPath = Join-Path $backupPath "20.jar.before_activity_phase1"
Copy-Item -LiteralPath $JarPath -Destination $jarBackupPath
Copy-Item -LiteralPath $ActivityPropertiesPath -Destination (Join-Path $backupPath "activity.properties.before_activity_phase1")
...
$checkSql = "SELECT " + "(SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='player' AND column_name='data_activity')," + "(SELECT COUNT(*) FROM activity_config_version WHERE version_no=1)," + "(SELECT COUNT(*) FROM activity_runtime_config WHERE id=1);"
$checkArgs = @(
    "--protocol=TCP", "--host=$($database.Host)", "--port=$($database.Port)", "--user=$($database.User)",
    "--batch", "--skip-column-names", "--execute=$checkSql", $database.Name
)
$check = ((Invoke-MySqlProgram -Executable $mysql -Arguments $checkArgs -Password $database.Password) -join "").Trim().Split("`t")
if ($check.Count -ne 3 -or [int]$check[0] -ne 1 -or [int]$check[1] -ne 1 -or [int]$check[2] -ne 1) {
    throw "Activity Phase 1 schema verification failed."
}

$manifest = [ordered]@{
    phase = "activity-phase1"
    ...
    dataActivityColumnPresent = $true
    configVersionBaselinePresent = $true
    runtimeRevisionBaselinePresent = $true
}
