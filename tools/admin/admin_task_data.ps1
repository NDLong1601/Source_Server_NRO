...
function List-TaskConfig {
    $path = Get-AdminConfigPath "task.properties"
...
function Save-TaskConfig {
    ...
    if ($entry.Key -in @("task.kanao.killCountMin", "task.kanao.killCountMax")) {
        $map = Get-PropertyMap (Get-AdminConfigPath "task.properties")
    ...
    Set-PropertyValue -Path (Get-AdminConfigPath "task.properties") -Key $entry.Key -Value $validated
...
function Reset-TaskConfig {
    ...
    Set-PropertyValue -Path (Get-AdminConfigPath "task.properties") -Key $entry.Key -Value "" -Remove
...
function Save-KanaoTaskConfig {
    ...
    Set-PropertyValues -Path (Get-AdminConfigPath "task.properties") -Entries @(
