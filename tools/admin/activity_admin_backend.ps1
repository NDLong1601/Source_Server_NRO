...
function Get-ActivityOverview {
    ...
    $activityProperties = Get-PropertyMap (Get-AdminConfigPath 'activity.properties')
    [ordered]@{
...
function Set-ActivityEmergencyDisable {
    ...
    $path = Get-AdminConfigPath 'activity.properties'
    ...
    [ordered]@{ status='ok'; disabled=$disabled; revision=$runtime.revision; message='Đã đổi emergency flag; server poll config/activity.properties trong tối đa 5 giây.' } | ConvertTo-Json
}
