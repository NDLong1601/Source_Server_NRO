...
import nro.models.player.PlayerPersistenceToken;
...
public final class PlayerSnapshotMapper {
    ...
    public static void updateEventRankingPoints(Player player) {
        ...
        try {
            ...
            Logger.logException(PlayerSnapshotMapper.class, e, "Không thể đồng bộ điểm bảng xếp hạng sự kiện");
        }
    }
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            Logger.logException(PlayerSnapshotMapper.class, e, "Lỗi tạo player mới");
            return false;
        }
    }

    public static PlayerSnapshot capture(Player player) {
        if (player == null || player.inventory == null || player.idMark == null) {
            return null;
        }
        if (player.isPersistenceQuarantined()) {
            Logger.error("[SEC-04] Skipped player snapshot for quarantined playerId=" + player.id);
            return null;
        }
        if (player.idMark.isLoadedAllDataPlayer()) {
            synchronized (player.getLifecycleLock()) {
              synchronized (player.inventory) {
                if (player.isPersistenceQuarantined()) {
                    Logger.error("[SEC-04] Skipped player snapshot after quarantine race, playerId=" + player.id);
                    return null;
                }
                try {
                JSONArray dataArray = new JSONArray();
                ...
                // data event
                String dataEvent = player.event.state().toEventJson();
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "rank = ?, data_achievement = ?, giftcode = ?, event_point = ?, data_event = ?, dataBadges = ?, hide_badges = ?, dataTaskBadges = ?, BoughtSkill = ?, LearnSkill = ?, "
                        ...
                        + "save_version = save_version + 1 where id = ? and save_version = ?";
                PlayerPersistenceToken persistenceToken = player.getPersistenceState().captureToken();
                Object[] parameters = new Object[]{
                        ...
                        player.id,
                        persistenceToken.expectedSaveVersion()};
                return new PlayerSnapshot(player.id, player.name, player.isOffline,
                        persistenceToken.expectedSaveVersion(), PlayerPersistenceSchema.CURRENT_SCHEMA_VERSION,
                        query, parameters, persistenceToken,
                        new PlayerWalletSnapshot(inventorySnapshot.getGold(), inventorySnapshot.getGem(),
                                player.inventory.ruby, player.inventory.coupon),
                        captureSuperRank(player));
                } catch (Exception e) {
                    Logger.logException(PlayerSnapshotMapper.class, e, "Lỗi snapshot player " + player.name);
                    return null;
                }
    ...
    private static PlayerRankSnapshot captureSuperRank(Player player) {
        if (player.superRank == null || player.nPoint == null) {
            return null;
        }
        JSONArray historyList = new JSONArray();
        int historySize = Math.min(player.superRank.history.size(), player.superRank.lastTime.size());
        for (int index = 0; index < historySize; index++) {
            JSONObject history = new JSONObject();
            history.put("event", player.superRank.history.get(index));
            history.put("timestamp", player.superRank.lastTime.get(index));
        ...
        return new PlayerRankSnapshot(player.superRank.rank, player.name, info.toJSONString(),
                player.superRank.lastPKTime, player.superRank.lastRewardTime,
                player.superRank.ticket, player.superRank.win, player.superRank.lose,
                historyList.toJSONString());
    }
