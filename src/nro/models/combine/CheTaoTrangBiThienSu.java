...
public class CheTaoTrangBiThienSu {
    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() < 4 || player.combineNew.itemsCombine.size() > 4) {
            ...
            return;
        } else if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isCongThucVip())
                .count() != 1) {
            Service.gI().sendThongBao(player, "Thiếu Công Thức Vip");
            return;
        } else if (player.combineNew.itemsCombine.stream()
                .filter(item -> item.isNotNullItem() && item.isManhTS() && item.quantity >= 999).count() != 1) {
            Service.gI().sendThongBao(player, "Thiếu Mảnh Thiên Sứ");
            return;
        } else if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaNangCap1())
                .count() != 1) {
            Service.gI().sendThongBao(player, "Thiếu Đá Nâng Cấp");
            return;
        } else if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaMayMan())
                .count() != 1) {
            ...
            if (Util.isTrue((float) Math.min(100, tilemacdinh), 100)) {
                ...
                if (tilemacdinh > 0) {
                    for (byte w = 0; w < itemTS.itemOptions.size(); w++) {
                        if (itemTS.itemOptions.get(w).optionTemplate.id != 0
                                && itemTS.itemOptions.get(w).optionTemplate.id != 20) {
                ...
                if (tilemacdinh <= tileLucky) {
                    ...
                    for (int j = 0; j < tileLucky; j++) {
                        ...
                        itemTS.itemOptions.add(
                                new ItemOption(listOptionBonus.get(tilemacdinh), Util.nextInt(paramMin, paramMax)));
    ...
    public static void CheTaoTS(Player player) {
        ...
        Item itemTL = player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDHD())
                .findFirst().get();
        Item itemManh = player.combineNew.itemsCombine.stream()
