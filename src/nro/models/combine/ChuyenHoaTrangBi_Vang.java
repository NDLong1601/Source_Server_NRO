...
public class ChuyenHoaTrangBi_Vang {
    ...
    public static void thucHienChuyenHoa(Player player) {
        ...
        player.getWallet().tryDebit(Currency.GOLD, goldChuyenHoa, WalletMutationContext.of(WalletReason.COMBINE_FEE, "Chuyển hóa trang bị vàng")).requireSuccess();
    ...
    private static boolean isTrangBiGoc(Item item) {
        return isDoLuongLong(item) || isDoJean(item) || isDoZelot(item) || isDoThanXD(item) || isDoThanTD(item)
                || isDoThanNM(item);
    }
